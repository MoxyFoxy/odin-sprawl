...
struct_index :: proc (ndim: Sprawl_Slice($T, $NT), indexes: []NT) -> NT {
    return index(ndim.slice, ndim.lengths);
}

// Abstraction over `index_2d` for the `Sprawl_Slice` struct
struct_index_2d :: proc (ndim: Sprawl_Slice($T, $NT), x, y: NT) -> NT {
    return index_2d(ndim.slice, x, y, ndim.lengths[0]);
}

// Abstraction over `_get` for the `Sprawl_Slice` struct
struct_get :: proc (ndim: Sprawl_Slice($T, $NT), indexes: []NT) -> T {
    return _get(ndim.slice, indexes);
}

// Abstraction over `_get_2d` for the `Sprawl_Slice` struct
struct_get_2d :: proc (ndim: Sprawl_Slice($T, $NT), x, y: NT) -> T {
    return _get_2d(ndim.slice, x, y, ndim.lengths[0]);
}

// Abstraction over `_set` for the `Sprawl_Slice` struct
struct_set :: proc (ndim: Sprawl_Slice($T, $NT), indexes: []NT, value: T) {
    _set(ndim.slice, indexes, ndim.lengths, value);
}

// Abstraction over `_set_2d` for the `Sprawl_Slice` struct
struct_set_2d :: proc (ndim: Sprawl_Slice($T, $NT), x, y: NT, value: T) {
    _set_2d(ndim.slice, x, y, ndim.lengths[0], value);
}

// Abstraction over `_in_bounds` for the `Sprawl_Slice` struct
struct_in_bounds :: proc (ndim: Sprawl_Slice($T, $NT), indexes: []NT) -> bool {
    return _in_bounds(ndim.slice, indexes, ndim.lengths);
}

// Abstraction over `_in_bounds_2d` for the `Sprawl_Slice` struct
struct_in_bounds_2d :: proc (ndim: Sprawl_Slice($T, $NT), x, y: NT) -> bool {
    return _in_bounds(ndim.slice, x, y, ndim.lengths);
}

// Creates a `Sprawl_Slice` and initializes the sprawled slice
create_sprawl :: proc (lengths: []$NT, $T: typeid) -> Sprawl_Slice(T, NT) {
    return Sprawl_Slice(T, NT) {
        create_slice(lengths),
        lengths,
    };
}

// Creates a constant-sized `Sprawl_Slice` and initializes it
create_sprawl_constant :: proc (lengths: [$N]$NT, $T: typeid) -> Sprawl_Slice(T, NT) {
    ndim := Spraw_Slice(T, NT) {
        create_slice(sizex, sizey, T),
        make([]NT, 2),
    }
...
delete_sprawl :: proc (ndim: ^Sprawl_Slice($T, $NT)) {
    delete(ndim.slice);
}

// Copies/clones a `Sprawl_Slice`, including its slice data
copy_sprawl :: proc (ndim: Sprawl_Slice($T, $NT)) {
